...
通用大模型回答医学问题时有两个系统性风险：

1. **幻觉**：编造不存在的文献、数据或 PMID
2. **假共识**：把文献里本就对立的结论抹平，自信地给出一个"确定"答案

而循证医学的根基恰恰相反：当证据存在冲突时，要把它讲清楚，而不是假装不存在。医学编辑、医生、科研人员查文献，很多时候就是为了搞清楚"这件事到底有没有争议"。这个产品把这件事显式地做了出来，而不是靠模型"顺便"输出。
...
**结论句抽取**：优先取摘要里带 `CONCLUSIONS` / `FINDINGS` / `INTERPRETATION` 标签的句子；跳过 `DISCUSSION` / `LIMITATION` 段；都没有则取末句。

**极性判定**用三层规则（强短语优先，否定词感知），见 `app/evidence.py`：
...
为什么需要"否定词感知"：像 `no significant benefit` 这种句子，如果只做正负关键词计数，会同时命中 `benefit`（正）和 `no significant`（负），打成平手被误判为中立。显式识别否定词后才正确判为反对。

#### 2. 证据冲突检测

统计所有结论句的极性，按下面的规则给出总体判定：

- `support > 0` 且 `against > 0` → **证据存在分歧**
- 只有 `support` → **证据倾向支持**
- 只有 `against` → **证据倾向不支持**
- 其余 → **证据不足或未明确**

当存在分歧时，前端会高亮提示"文献结论存在对立，采信需谨慎"，并把正反两边的 PMID 数量都列出来。

#### 3. 证据置信度评分（0-100）

置信度是一个可解释的启发式评分，不是黑箱：

```
置信度 = 证据等级基础分 ± 一致性 ± 文献量 ± 时效
```

| 分项 | 规则 |
| --- | --- |
| 证据等级基础分 | High 90 / Moderate 70 / Low 50 / Very low 30 / N/A 40，取文献中的最高档 |
| 结论一致性 | 无冲突 +10，有冲突 -15 |
| 文献量 | ≥6 篇 +8，≥3 篇 +5 |
| 时效 | 近 5 年内有文献 +5 |

结果钳制到 0-100。分数旁边会同时展示"支持 / 反对 / 中立"的篇数，所以这个分数是可拆解、可质疑的，而不是一个凭空蹦出来的数。

### 一个真实例子

问「他汀类药物会增加新发糖尿病风险吗」，系统检索并分析后输出：

- **判定：证据存在分歧**（支持 1 / 反对 4 / 中立 3，置信度 88）
- "反对"来自多篇 meta 分析，如 Sattar 2010（PMID 20167359），指出他汀轻微增加新发糖尿病风险
- "支持"来自他汀降低心血管事件的证据

这正是该领域的真实张力：他汀降低心血管事件，却轻微增加糖尿病风险。通用大模型可能直接说"他汀利大于弊"，而这个产品把两边的证据都摆出来，让使用者自己判断。
...
- 极性和分级是规则驱动，不是语义理解，复杂句式（如 `not associated with increased risk` 这类双重否定）可能误判
- 置信度是启发式评分，不是严格统计推断，也没有做效应量的 meta 合并
- 抽取式回答只选择并标注已有句子，不生成新的推理

这些局限恰好指向清晰的后续方向：向量语义检索替换 BM25、PICO（人群 / 干预 / 对照 / 结局）结构化、效应量合并做真正的定量 meta 分析。
